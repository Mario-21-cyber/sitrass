...
<div style="background:var(--white); border:1px solid var(--border); border-radius:8px; padding:1.5rem; margin-bottom:1.5rem;">
    <p><strong>Kasalukuyang Petsa:</strong> <?= htmlspecialchars($booking['travel_date']) ?> @ <?= htmlspecialchars($booking['pickup_time']) ?></p>
    <p><strong>Bilang ng Pasahero:</strong> <?= (int)$booking['seats_booked'] ?></p>
</div>

<?php if (empty($alternatives)): ?>
    <p>Walang ibang available na biyahe sa rutang ito sa ngayon.</p>
<?php else: ?>
    <form method="POST" action="/sitrass/public/customer/confirmReschedule">
        ...
        <?php foreach ($alternatives as $alt): ?>
            <label style="display:block; background:var(--white); border:1px solid var(--border); border-radius:8px; padding:1rem; margin-bottom:0.75rem; cursor:pointer;">
                ...
                &middot; <?= (int)$alt['available_seats'] ?> upuang bakante
                &middot; <?= htmlspecialchars($alt['plate_number']) ?>
            </label>
        <?php endforeach; ?>

        <button type="submit" class="btn" style="width:auto; padding:0.7rem 2rem; margin-top:1rem;">Kumpirmahin ang Bagong Petsa</button>
    </form>
<?php endif; ?>
