...
<div style="background:var(--white); border:1px solid var(--border); border-radius:8px; padding:1.5rem; margin-bottom:1.5rem;">
    ...
    <p><strong>Balance:</strong> ₱<?= htmlspecialchars($reservation['balance_due']) ?></p>
</div>

<?php foreach ($methods as $m): ?>
    <div class="method-details" id="details_<?= (int)$m['method_id'] ?>" style="display:none; background:var(--teal-dark); color:var(--white); border-radius:8px; padding:1.25rem; margin-bottom:1.5rem;">
        <?php if ($m['is_online'] && $m['account_number']): ?>
            <div class="stub-label" style="color:var(--amber); font-size:0.75rem; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:0.4rem;">
